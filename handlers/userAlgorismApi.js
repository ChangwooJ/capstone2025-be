...
const getAlgorithmLogs = async (req, res) => {
    try {
        const response = await axios.get('http://13.211.209.54:8000/logs');
        res.status(200).json(response.data);
    } catch (error) {
...
const startAiTrading = async (req, res) => {
    try {
        const response = await axios.post('http://13.211.209.54:8000/start');
        res.status(200).json(response.data);
        console.log("거래 시작");
    } catch (error) {
...
const stopAiTrading = async (req, res) => {
    try {
        const response = await axios.post('http://13.211.209.54:8000/stop');
        res.status(200).json(response.data);
        console.log("거래 중지");
    } catch (error) {
...
const getAiStatus = async (req, res) => {
    try {
        const response = await axios.get('http://13.211.209.54:8000/status');
        res.status(200).json(response.data);
    } catch (error) {
        console.error('알고리즘 로그 조회 중 오류 발생:', error);
        res.status(500).json({
